    ...
    <div class="shrink-0 z-10 bg-white">
      ...
      <div class="pt-0">
        <div class="flex items-center pb-2">
          <input type="checkbox" id="agree" :checked="isAgreed" @click="handleCheckboxClick" class="mr-2 h-4 w-4 text-rose-400 border-neutral-300 rounded focus:ring-0 focus:outline-none focus:ring-transparent" />
          <label for="agree" class="text-sm text-neutral-700">
            I agree to to the terms of the disclaimer.
          </label>
        </div>
        <Disclaimer v-show="isDisclaimerVisible" @close="closeDisclaimer"/>
        <div class="flex justify-end space-x-4">
          ...
          <button @click="applyAllAllergies" :disabled="!isAgreed" class="px-4 w-24 h-11 border-1 border-rose-400 text-rose-400 rounded-full hover:text-white hover:bg-rose-400 transition disabled:opacity-50 " :title="!isAgreed ? 'Please agree to continue.' : ''">Apply</button>
        </div>
      </div>
    </div>
  </div>
